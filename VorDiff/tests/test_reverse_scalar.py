...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  9 15:49:01 2019
...
def test_pow():
    ...
    assert round(h_2.get()[1],5) == round(2**f.get()[0]*np.log(2)*f.get()[1],5)
    """test the value of function to the power of function for both value and derivative"""
    assert h_3.get()[0] == f.get()[0]**g.get()[0]
    assert round(h_3.get()[1],5) == round(h_3.get()[0]*(np.log(f.get()[0])*g.get()[1]+g.get()[0]*f.get()[1]/f.get()[0]),5)


def test_divide():
    """test division (f/g) for both value and derivative"""
    assert f_7.get()[0] == f.get()[0] / g.get()[0]
    assert round(f_7.get()[1],5) == (f.get()[1]*g.get()[0]-f.get()[0]*g.get()[1]) / g.get()[0]**2
